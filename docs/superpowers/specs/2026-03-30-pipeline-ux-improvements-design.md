...
**Error handling**: Best-effort. If the AI call fails, log a warning and continue with the intent from the pre-ingest question. The pipeline is not blocked.
...
Run chains: `[pre-ingest questions] → ingest → [Pass 1] → analyze → [Pass 2] → generate → export → [toolkit wizard]`
...
```typescript
const RUN_STEPS = [
  { id: 'intent-type', phase: 'empty', interactive: true },     // pre-ingest cold questions
